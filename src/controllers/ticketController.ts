...
export class TicketController {
  ...
  static async getTicketById(req: Request, res: Response): Promise<void> {
    try {
      ...
      const hasAccess = 
        ...
        ticket.assignedDepartments.includes(userDepartment as string) ||
  ...
  static async updateTicket(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Check access control
      const hasAccess = 
        existingTicket.isOpenTicket ||
        existingTicket.department === userDepartment ||
        existingTicket.assignedDepartments.includes(userDepartment as string) ||
  ...
  // Add activity log entry
  static async addActivityLog(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
  ...
  static async getTicketsByDepartment(req: Request, res: Response): Promise<void> {
    try {
  ...
  static async getTicketsByAsset(req: Request, res: Response): Promise<void> {
    try {
      const { assetId } = req.params;
      const { status, limit = 50 } = req.query;

      const query: any = { equipmentId: assetId };
  ...
  static async getTicketStats(req: Request, res: Response): Promise<void> {
    try {
      ...
      const [
        ...
        Ticket.countDocuments({ ...baseQuery, priority: 'Critical' }),
        Ticket.aggregate([
          { $match: baseQuery },
          {
            $group: {
