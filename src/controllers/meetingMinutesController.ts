...
import MeetingMinutes, { IMeetingMinutes } from '../models/MeetingMinutes';
import { AuthenticatedRequest } from '../middleware/authMiddleware';

export class MeetingMinutesController {
  // Get all meeting minutes with department-based filtering
  static async getAllMeetingMinutes(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const {
        page = 1,
        limit = 10,
      ...
      if (dateFrom || dateTo) {
        ...
        if (dateFrom) {
          query.meetingDateTime.$gte = new Date(dateFrom as string);
        }
        if (dateTo) {
          query.meetingDateTime.$lte = new Date(dateTo as string);
        }
      }
  ...
  static async getMeetingMinutesById(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;

      const meetingMinutes = await MeetingMinutes.findById(id).lean();
  ...
  static async createMeetingMinutes(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      // Create new meeting minutes
      const newMeetingMinutes = new MeetingMinutes({
        title,
  ...
  static async updateMeetingMinutes(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      const existingMeetingMinutes = await MeetingMinutes.findById(id);
  ...
  static async deleteMeetingMinutes(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
  ...
  static async getMeetingMinutesStats(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const [
        ...
        MeetingMinutes.countDocuments(departmentFilter),
        MeetingMinutes.countDocuments({ ...departmentFilter, status: 'published' }),
        MeetingMinutes.countDocuments({ ...departmentFilter, status: 'draft' }),
        MeetingMinutes.countDocuments({ ...departmentFilter, status: 'archived' }),
        MeetingMinutes.aggregate([
          { $match: departmentFilter },
          { $group: { _id: '$department', count: { $sum: 1 } } },
          { $sort: { count: -1 } },
        ]),
        MeetingMinutes.find(departmentFilter)
  ...
  static async updateActionItemStatus(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      const actionItemIndex = meetingMinutes.actionItems.findIndex(
        (item: any) => item._id?.toString() === actionItemId
      );
