...
const calculateNextDueDate = (frequency: string, startDate: Date, customDays?: number): Date => {
...
export const getSafetyInspectionSchedules = async (req: Request, res: Response) => {
  try {
    const {
      page = 1,
      limit = 10,
    ...
    if (assignedInspector) filter.assignedInspector = { $regex: assignedInspector, $options: 'i' };
    if (safetyStandard) filter.safetyStandards = { $in: [safetyStandard] };
    ...
    const [schedules, totalCount] = await Promise.all([
      SafetyInspectionSchedule.find(filter)
        .sort(sort)
        .skip(skip)
        .limit(limitNum)
        .lean(),
      SafetyInspectionSchedule.countDocuments(filter)
    ]);
...
export const getSafetyInspectionScheduleById = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const schedule = await SafetyInspectionSchedule.findById(id);
...
export const createSafetyInspectionSchedule = async (req: Request, res: Response) => {
  try {
    ...
    scheduleData.nextDueDate = calculateNextDueDate(
      scheduleData.frequency,
      new Date(scheduleData.startDate),
      scheduleData.customFrequencyDays
    );

    const schedule = new SafetyInspectionSchedule(scheduleData);
    await schedule.save();
...
export const updateSafetyInspectionSchedule = async (req: Request, res: Response) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
    ...
    if (updates.frequency || updates.startDate || updates.customFrequencyDays) {
      ...
      if (schedule) {
        updates.nextDueDate = calculateNextDueDate(
          ...
          new Date(updates.startDate || schedule.startDate),
          updates.customFrequencyDays || schedule.customFrequencyDays
        );
      }
    }

    const schedule = await SafetyInspectionSchedule.findByIdAndUpdate(
      id,
      updates,
...
export const deleteSafetyInspectionSchedule = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;

    const schedule = await SafetyInspectionSchedule.findByIdAndDelete(id);
    ...
    await SafetyInspectionRecord.deleteMany({ scheduleId: id });
...
export const getSafetyInspectionStats = async (req: Request, res: Response) => {
  try {
    const now = new Date();
    ...
    const [
      ...
      SafetyInspectionSchedule.countDocuments(),
      SafetyInspectionSchedule.countDocuments({ status: 'active' }),
      SafetyInspectionSchedule.countDocuments({ status: 'overdue' }),
      SafetyInspectionRecord.countDocuments({
...
export const getSafetyInspectionRecords = async (req: Request, res: Response) => {
  try {
    const {
      page = 1,
      limit = 10,
    ...
    if (inspector) filter.inspector = { $regex: inspector, $options: 'i' };
    
    if (dateFrom || dateTo) {
      filter.completedDate = {};
      if (dateFrom) filter.completedDate.$gte = new Date(dateFrom as string);
      if (dateTo) filter.completedDate.$lte = new Date(dateTo as string);
    }
    ...
    const [records, totalCount] = await Promise.all([
      SafetyInspectionRecord.find(filter)
        .sort(sort)
        .skip(skip)
        .limit(limitNum)
        .lean(),
      SafetyInspectionRecord.countDocuments(filter)
    ]);
...
export const createSafetyInspectionRecord = async (req: Request, res: Response) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
    ...
    const record = new SafetyInspectionRecord(req.body);
    await record.save();

    // Update the schedule's last completed date
    await SafetyInspectionSchedule.findByIdAndUpdate(
      record.scheduleId,
...
export const updateSafetyInspectionRecord = async (req: Request, res: Response) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
    ...
    const { id } = req.params;
    const record = await SafetyInspectionRecord.findByIdAndUpdate(
      id,
      req.body,
      { new: true, runValidators: true }
    );
...
export const verifySafetyInspectionRecord = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
