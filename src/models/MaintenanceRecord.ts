...
const MaintenanceChecklistRecordSchema = new Schema<IMaintenanceChecklistRecord>(
  {
    itemId: {
      type: String,
...
const MaintenancePartRecordSchema = new Schema<IMaintenancePartRecord>(
  {
    partId: {
      type: String,
...
MaintenanceRecordSchema.virtual('completionPercentage').get(function() {
  if (!this.partsStatus || this.partsStatus.length === 0) return 0;
  
  const totalItems = this.partsStatus.reduce((sum, part) => sum + part.checklistItems.length, 0);
  const completedItems = this.partsStatus.reduce((sum, part) => 
...
MaintenanceRecordSchema.set('toJSON', {
  transform: function(doc, ret) {
    ...
    if (ret.partsStatus) {
      ret.partsStatus = ret.partsStatus.map((part: any) => {
        part.id = part._id;
        delete part._id;
        
        if (part.checklistItems) {
          part.checklistItems = part.checklistItems.map((item: any) => {
            item.id = item._id;
            delete item._id;
            return item;
          });
